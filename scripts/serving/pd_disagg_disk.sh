...
# with LocalDisk as L2 (CPU DRAM = L1 staging, disk = L2).
#
# Based on tests/run_pd_disagg_deepseek_v4.sh; LMCache YAML adds local_cpu +
# local_disk so both P and D can persist/reuse KV beyond the PD HCCL path.
#
...
#
# Multi-node: same PREFILL_HOST/DECODE_HOST/PROXY_HOST/LAUNCH_PROXY/NIC_NAME/LOCAL_IP
# env as run_pd_disagg_deepseek_v4.sh. Note: L2 disk is per-node unless the path
# is a shared filesystem; cross-node KV still goes over HCCL PD.
#
# Usage:
#   ./run_pd_disagg_deepseek_v4_disk.sh [all|launch|test|stop] [--debug]
#
# Optional env (in addition to the PD baseline):
#   PREFILL_HOST, DECODE_HOST, PROXY_HOST, DECODE_BIND_HOST, LAUNCH_PROXY
#   NIC_NAME, LOCAL_IP (RoCE/HCCL for multi-node)
#   LMCACHE_LOCAL_DISK / LMCACHE_MAX_LOCAL_DISK_SIZE — L2 disk path & size (GB)
#   LMCACHE_MAX_LOCAL_CPU_SIZE — L1 CPU staging size (GB)
#   PD_INSTANCE: prefill | decode | both (default both for L2 reuse tests)
...
PD_INSTANCE="${PD_INSTANCE:-both}"   # prefill | decode | both

if [[ "$PREFILL_HOST" != "$DECODE_HOST" && "$DECODE_NPUS" == "8,9,10,11,12,13,14,15" ]]; then
    DECODE_NPUS="0,1,2,3,4,5,6,7"
fi

# L1 (CPU) staging + L2 (LocalDisk). Shared path so P/D on one node can reuse.
LMCACHE_LOCAL_CPU="${LMCACHE_LOCAL_CPU:-True}"
LMCACHE_MAX_LOCAL_CPU_SIZE="${LMCACHE_MAX_LOCAL_CPU_SIZE:-70}"
# Plain path or file:// URL (file:// stripped when writing YAML).
LMCACHE_LOCAL_DISK="${LMCACHE_LOCAL_DISK:-/tmp/lmcache_disk_pd}"
LMCACHE_MAX_LOCAL_DISK_SIZE="${LMCACHE_MAX_LOCAL_DISK_SIZE:-500}"
LMCACHE_DISK_PATH="${LMCACHE_LOCAL_DISK#file://}"

LOG_DIR="${LOG_DIR:-${REPO_ROOT}/logs/pd_disagg_dsv4_disk}"
CFG_DIR="${CFG_DIR:-${LOG_DIR}/configs}"
PID_FILE="${PID_FILE:-${LOG_DIR}/pd_disagg_dsv4_disk.pids}"
...
PREFILL_CFG="${PREFILL_LMCACHE_CONFIG:-${CFG_DIR}/lmcache-dsv4-prefiller-disk-config.yaml}"
DECODE_CFG="${DECODE_LMCACHE_CONFIG:-${CFG_DIR}/lmcache-dsv4-decoder-disk-config.yaml}"
...
write_lmcache_configs() {
    mkdir -p "$CFG_DIR" "$LMCACHE_DISK_PATH"
...
# L1 staging + L2 disk (same path as prefiller for single-node reuse;
# on multi-node use a shared FS or accept per-node L2).
local_cpu: ${LMCACHE_LOCAL_CPU}
max_local_cpu_size: ${LMCACHE_MAX_LOCAL_CPU_SIZE}
local_disk: "${LMCACHE_DISK_PATH}"
max_local_disk_size: ${LMCACHE_MAX_LOCAL_DISK_SIZE}
...
EOF
    ...
    echo "  L2 disk: ${LMCACHE_DISK_PATH} (max ${LMCACHE_MAX_LOCAL_DISK_SIZE} GB)"
...
launch_all() {
    ...
    echo "L1 CPU: ${LMCACHE_LOCAL_CPU} max=${LMCACHE_MAX_LOCAL_CPU_SIZE}GB  L2 disk: ${LMCACHE_DISK_PATH} max=${LMCACHE_MAX_LOCAL_DISK_SIZE}GB"
    ...
    if [[ "$PD_INSTANCE" == "both" && "$PREFILL_HOST" != "$DECODE_HOST" ]]; then
        ...
        echo "         Prefer PD_INSTANCE=prefill / decode on each node."
    fi
